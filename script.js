...
//https://platzi.com.br/blog/jogo-da-velha-javascript/
// https://edrodrigues.com.br/blog/crie-o-jogo-da-velha-simples-usando-html-css-javascript/
let classe_botao = []
let nome_celula = []
...
//document.getElementById('caixa_conteudo-id').classList.toggle("caixa-conteudo-class")
...
for (let i = 0; i < 9; i++) {
	var id_celula = 'celula' + i
	classe_botao[i] = document.getElementById(id_celula).className

}



console.log(classe_botao[0])
console.log(typeof (classe_botao[0]))
...
function casa_0() { // Quando clicar em um input, será atribuído O ou X por essa function
	...
	if (numero_de_jogadas == 9 && vencedor == '_') {
		...
		const empate=new Audio('som/musica-empate-mario.mp3');
		empate.play();
		console.log('Empate')
...
function verificador_vitoria() {
	if ((casas[0].value == casas[1].value) && (casas[1].value == casas[2].value) && casas[0].value != '_' && existe_vencedor == false) {
		...
		numero_de_jogadas = 0
		document.getElementById('caixa_conteudo-id').classList.toggle("caixa-conteudo-class-not")
		const vitoria=new Audio('som/musica-vitoria-mario.mp3');
		vitoria.play();
		setTimeout(function(){
